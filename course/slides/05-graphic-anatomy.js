export const slide = {
  render() {
    ...
    const zoneRows = zones.map(function(zone, i) {
      ...
      return '<div style="display:flex;gap:14px;align-items:flex-start;padding:13px 0;'
        + (isLast ? '' : 'border-bottom:1px solid #f0f0f0;') + '">'
        + '<div style="width:28px;height:28px;border-radius:50%;background:#CC0000;color:#fff;'
        ...
        + '<div style="flex:1;">'
        + '<div style="font-weight:700;color:#1A1A1A;font-size:0.9rem;margin-bottom:3px;">' + zone.label + '</div>'
        + '<div style="color:#666;font-size:0.83rem;line-height:1.6;margin-bottom:5px;">' + zone.desc + '</div>'
        + '<div style="font-size:0.78rem;color:#CC0000;font-weight:600;">'
        + '<span style="color:#bbb;font-weight:400;margin-right:4px;">e.g.</span>' + zone.eg
        + '</div>'
        + '</div>'
        + '</div>';
    }).join('');
    ...
    el.innerHTML = `
      <div style="max-width:800px;margin:0 auto;padding:32px 24px;font-family:sans-serif;">
        ...
        <div style="border-top:1px solid #ebebeb;margin-bottom:32px;">${zoneRows}</div>
